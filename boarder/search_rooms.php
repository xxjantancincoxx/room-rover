...
$totalRecords = $resultTotal->fetch(PDO::FETCH_ASSOC)['total'];
                    ...
                    <div class="col-md-4"   style="padding: 30px;">
                        <div class="card hovercard text-center" >
                            <div class="cardheader">
                                ...
                                <div class="avatar">
                                    <img alt="" src="<?php echo $BASE_URL . 'backend/api/uploads/OWNER' . $record['owner_id'] . '/' . $record['pic']; ?>" class="img-fluid" style="max-height: 200px; max-width: 100%;">
                                </div>
                            </div>
                            <div class="card-body info">
                                <div class="title">
                                    <a href="#" style="font-size: 18px; font-weight: bold; color: #333;"><?php echo $record['name']; ?></a>
                                </div>
                                ...
                                <div class="desc text-left">
                                    <strong>E-wallet/Contact Number:</strong> <?php echo $record['e_wallet']; ?>
                                </div>
                               <div class="desc text-left">
                                    <strong>Condition:</strong> <?php echo $record['is_aircon'] ? 'Air Conditioned' : 'No Air Condition'; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>Water & Electric:</strong> <?php echo $record['free_water_electric'] ? 'Free Water and Electric' : 'No Free Water and Electric'; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>WiFi:</strong> <?php echo $record['free_wifi'] ? 'Free WiFi' : 'No Free WiFi'; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>Own CR:</strong> <?php echo $record['own_cr'] ? 'Own CR' : 'No Own CR'; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>Rooms Available:</strong> <?php echo $record['rooms_Available']; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>Number of Reviews:</strong> <?php echo $record['review_count']; ?>
                                </div>
                                <div class="desc text-left">
                                    <strong>Rating:</strong>
                                    <!-- Use the average_rating column in the star-indicator data-rating attribute -->
                                    <div class="star-indicator" data-rating="<?php echo $record['average_rating']; ?>"></div>
                                    <!-- Display the average rating value -->
                                    <span><?php echo number_format($record['average_rating'], 2); ?></span>
                                </div>

                            </div>
                            <div class="card-footer bottom d-flex justify-content-between align-items-center">
    <!-- Update Reserve Button -->
    <a class="btn btn-primary btn-reserve btn-sm reserve-button" 
        href="#" 
        data-toggle="modal" 
        data-target="#reserveModal"
        data-card-id="<?php echo $record['listing_id']; ?>"
        data-card-name="<?php echo $record['name']; ?>"
        data-card-loc="<?php echo $record['location']; ?>"
        data-card-price="<?php echo'₱' . $record['price']; ?>"
        data-card-condition="<?php echo $record['is_aircon'] ? 'Air Conditioned' : 'No Air Condition'; ?>"
        data-card-water-electric="<?php echo $record['free_water_electric'] ? 'Free Water and Electric' : 'No Free Water and Electric'; ?>"
        data-card-wifi="<?php echo $record['free_wifi'] ? 'Free WiFi' : 'No Free WiFi'; ?>"
        data-card-own-cr="<?php echo $record['own_cr'] ? 'Own CR' : 'No Own CR'; ?>"
        data-card-rooms-available="<?php echo $record['rooms_Available']; ?>">
        Reserve
    </a>

    <!-- QR Code Button -->
    <button class="btn btn-warning btn-sm" onclick="showQrCodeModal('<?php echo $BASE_URL . 'backend/api/uploads/OWNER' . $record['owner_id'] . '/' . $record['qr_pic']; ?>')">
        <i class="fas fa-qrcode"></i> QR
    </button>

    <!-- Review Button -->
    <button class="btn btn-info btn-review btn-sm review-button" 
            data-toggle="modal" 
